...
export default function Focus({ 
  ...
  useEffect(() => {
    // Only set tasks initially or when the length changes significantly (e.g., loaded from backend)
    // To prevent the backend's default order (or lack thereof) from overwriting local drag-and-drop order
    setTasks(prev => {
      // If we already have the same tasks, just keep them (preserves local order)
      if (prev.length === initialTasks.length && prev.every(pt => initialTasks.find(it => it.id === pt.id))) {
        // Merge any background updates (like status changes from others) while keeping local order
        return prev.map(pt => {
           const updated = initialTasks.find(it => it.id === pt.id);
                ...
                <Droppable key={col.id} droppableId={col.id}>
                  {(provided) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-3 overflow-y-auto flex-1 min-h-[50px]">
